...
Commands:
- init-review: extract clause-like terms from a DOCX contract into a review JSON template.
...
def build_review_template(
    ...
    for index, paragraph in enumerate(paragraphs):
        ...
        if not current_title:
            current_title = f"Clause {clause_counter}"
            current_body = [text]
            current_indexes = [index]
            current_target_index = index
            continue
        ...
        if current_target_index == current_indexes[0]:
            current_target_index = index
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init_review = subparsers.add_parser(
        "init-review",
        help="Extract contract clauses into a JSON review template.",
    )
